...
const ApiKeyManager = ({
  ...
  return <div className="space-y-4">
      ...
      {needsOpenaiKey && <>
          {showOpenaiInput && <div className="p-4 border rounded-lg bg-muted/30">
              <label htmlFor="openaiApiKey" className="block text-sm font-medium mb-1">
                OpenAI API-Schlüssel:
              </label>
              <div className="flex gap-2">
                <input id="openaiApiKey" type="password" value={openaiInputValue} onChange={handleOpenaiKeyChange} className="flex-1 p-2 border rounded" placeholder="sk-..." disabled={disabled || saving} />
                <button onClick={saveOpenaiKey} disabled={disabled || saving} className="px-4 py-2 bg-gnb-primary text-white rounded hover:bg-gnb-secondary transition-colors disabled:opacity-50 flex items-center gap-2">
          ...
          {!showOpenaiInput && hasStoredOpenaiKey && <div className="p-3 border rounded-lg bg-green-50 text-green-800">
              ...
              setOpenaiInputValue(apiKeys.openai_api_key || '');
              setShowOpenaiInput(true);
            }} disabled={disabled || saving} className="text-xs px-2 py-1 border border-green-600 rounded hover:bg-green-100 transition-colors disabled:opacity-50">
                    Ändern
                  </button>
                  <button onClick={deleteOpenaiKey} disabled={disabled || saving} className="text-xs px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center gap-1">
          ...
          {!hasStoredOpenaiKey && !showOpenaiInput && <button onClick={() => setShowOpenaiInput(true)} disabled={disabled} className="w-full p-3 border-2 border-dashed border-gnb-primary text-gnb-primary rounded-lg hover:bg-gnb-primary/5 transition-colors disabled:opacity-50">
      ...
      {needsClaudeKey && <>
          {showClaudeInput && <div className="p-4 border rounded-lg bg-muted/30">
              <label htmlFor="claudeApiKey" className="block text-sm font-medium mb-1">
                Claude API-Schlüssel:
              </label>
              <div className="flex gap-2">
                <input id="claudeApiKey" type="password" value={claudeInputValue} onChange={handleClaudeKeyChange} className="flex-1 p-2 border rounded" placeholder="sk-ant-..." disabled={disabled || saving} />
                <button onClick={saveClaudeKey} disabled={disabled || saving} className="px-4 py-2 bg-gnb-primary text-white rounded hover:bg-gnb-secondary transition-colors disabled:opacity-50 flex items-center gap-2">
                  {saving && <Loader2 className="h-4 w-4 animate-spin" />}
                  Speichern
                </button>
              </div>
          ...
          {!showClaudeInput && hasStoredClaudeKey && <div className="p-3 border rounded-lg bg-blue-50 text-blue-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
              ...
              setClaudeInputValue(apiKeys.claude_api_key || '');
              setShowClaudeInput(true);
            }} disabled={disabled || saving} className="text-xs px-2 py-1 border border-blue-600 rounded hover:bg-blue-100 transition-colors disabled:opacity-50">
                    Ändern
                  </button>
                  <button onClick={deleteClaudeKey} disabled={disabled || saving} className="text-xs px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center gap-1">
                    {saving && <Loader2 className="h-3 w-3 animate-spin" />}
                    Löschen
                  </button>
                </div>
          ...
          {!hasStoredClaudeKey && !showClaudeInput && <button onClick={() => setShowClaudeInput(true)} disabled={disabled} className="w-full p-3 border-2 border-dashed border-gnb-primary text-gnb-primary rounded-lg hover:bg-gnb-primary/5 transition-colors disabled:opacity-50">
      ...
      <div className="text-xs text-muted-foreground">
        {needsClaudeKey}
        {needsOpenaiKey && selectedModel.startsWith('gpt-5-') && <p className="text-amber-600">
